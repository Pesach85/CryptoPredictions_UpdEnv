...
class BitmexDataset:
    ...
    def __init__(self, cfg):
        # Optional API credentials. For research-only local runs, CSV data can be used without credentials.
    ...
    def minutes_of_new_data(self, symbol, kline_size, data, source):
        ...
        if len(data) > 0:
            ...
            old = \
                self.bitmex_client.Trade.Trade_getBucketed(symbol=symbol, binSize=kline_size, count=1,
                                                           reverse=False).result()[
                    0][0]['timestamp']
    ...
    def get_all_bitmex(self, symbol, kline_size):
        ...
        # If no local data exists, fallback to historical API download.
        ...
        if rounds > 0:
            ...
            for round_num in tqdm_notebook(range(rounds)):
                ...
                data = \
                    self.bitmex_client.Trade.Trade_getBucketed(symbol=symbol, binSize=kline_size, count=self.batch_size,
    ...
    def create_dataset(self, df, window_size):
        ...
        arr = np.array(df)
        data, profit_calculator = create_dataset(arr, list(dates), look_back=window_size,
                                                         features=self.features)
        return data, profit_calculator

    def get_dataset(self):
        dataset = self.get_all_bitmex(self.symbol, self.bin)
        return dataset
