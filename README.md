...
This repository is intended for predictive-model experimentation and software benchmarking.
It is not intended to provide investment advice.
Backtesting outputs should be interpreted as simulation results only.
...
# 2026 Reproducible Example (How The Software Works)

This section shows a full, reproducible 2026 workflow using local CSV data and a leakage-safe split.
The goal is to demonstrate model-quality evaluation, not investment advice.

## 1) Run a leakage-safe 6-asset benchmark

Use training data up to `2025-12-31` and evaluate only on 2026 samples.
...
## 2) Run the heterogeneous per-asset strategy

The 2026 experiments showed that there is no universal winner, so different asset groups use different settings.

```bash
# Part 1: established/trending assets
python meta_historical_test.py --assets XBTUSD,LTCUSD --train-cutoff 2025-12-31 --min-samples 50 --max-mape 5.0 --n-estimators 300 --features close --lags 30

# Part 2: volatile/newer assets
python meta_historical_test.py --assets ETHUSD,ADAUSD,BCHUSD --train-cutoff 2025-12-31 --min-samples 50 --max-mape 5.0 --n-estimators 300 --features close --lags 14

# Part 3: SOL specialized mode
python meta_historical_test.py --assets SOLUSD --train-cutoff 2025-12-31 --min-samples 50 --max-mape 5.0 --n-estimators 300 --features focused --lags 14
```

## 3) 2026 Example Outcomes

- All six assets pass `MAPE <= 5.0`.
- Best predictable asset in this 2026 run: `LTCUSD` (MAPE `2.51%`, accuracy `0.562`, 106 eval samples).
- SOL improves materially with focused mode (MAPE `4.81% -> 4.21%`).

## 4) 2026 Charts Generated By The Pipeline

LTCUSD price prediction chart (daily actual vs predicted + weekly fluctuation):
...
- Per-run folders: `outputs/meta_historical/YYYY-MM-DD/HH-MM-SS/<ASSET>/`
- Key files per asset:
	- `meta_historical_report.json`
...
Fast check (validates the three README chart paths):

```bash
powershell -ExecutionPolicy Bypass -File scripts/run_2026_readme_example.ps1
```

Full rebuild + validation (reruns all 2026 example commands):

```bash
powershell -ExecutionPolicy Bypass -File scripts/run_2026_readme_example.ps1 -RunAll
```
